...
*/
private["_ownerID","_gangBank","_gangMax","_gangName","_members","_allUnits","_ctrl","_gangLeaders"];
...
_gangBank = grpPlayer getVariable ["gsn_gang_bank",0];
...
if(_ownerID != steamid) then {
	...
	(getControl(2620,2633)) ctrlEnable false; //add leader
};

if(steamid in _gangLeaders) then
{
	(getControl(2620,2630)) ctrlEnable true;
	(getControl(2620,2624)) ctrlEnable true;
	(getControl(2620,2622)) ctrlEnable true;
};
...
lbClear _members;
{
	...
	{
		if((getPlayerUID _x) in _gangLeaders) then
		{
			_members lbAdd format["%1 " +(localize "STR_GNOTF_GangCoLeader"),(_x getVariable["realname",name _x])];
		}
		else
		{
