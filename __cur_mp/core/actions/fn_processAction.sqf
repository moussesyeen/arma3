...
_itemInfo = switch (_type) do
{
	...
	case "grapes": {["pgrapes","vodka",200,"Processing Grapes into Vodka"]};
	case "moonshine": {["pgrapes","moonshine",650,"Processing Grapes into Moonshine"]};
	case "uranium": {["uranium","euranium",2050,"Enriching Uranium"]};
	case "sulfur": {["sulfur","sacid",1050,"Processing Sulfur"]};
	case "rfuel": {["oilu","rfuel",1025,"Distilling Oil"]};
	default {[];};
};
...
{
	if(gsn_life_cash < _cost) exitWith {hint format[localize "STR_Process_License",[_cost] call life_fnc_numberText]; 5 cutText ["","PLAIN"]; life_is_processing = false;};
	...
	[_cost,0,1] call life_fnc_handleMoney;
	life_is_processing = false;
