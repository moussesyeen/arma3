...
if(!gsn_use_atm) exitWith {hint "You recently robbed the bank! You can't give money away just yet.";ctrlShow[2001,true];};
...
if(parseNumber(_amount) > gsn_life_cash) exitWith {hint "You don't have that much to give!";ctrlShow[2001,true];};
if(isNull _unit) exitWith {ctrlShow[2001,true];};
...
[(parseNumber(_amount)),0,1] call life_fnc_handleMoney;
