	...
	if(!(lineIntersects [eyePos player, eyePos _x, player, _x]) && {!isNil {_x getVariable "realname"}}) then {
		...
		if(count _sPos > 1 && {_distance < 15}) then {
			_text = switch (true) do {
				...
				case (!isNil {(_x getVariable "rank")}): {format["<img image='%1' size='1'></img> %2<br/> %3",switch ((_x getVariable "rank")) do {
			...
			if(_x getVariable ["DirectVON",false]) then {_text = "<t color='#00FF88'>[SPEAKING] " + _text};
