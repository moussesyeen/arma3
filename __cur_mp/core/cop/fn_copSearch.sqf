...
{
	{
		...
		_index = [_x select 0,__GETC__(gsn_sell_array)] call TON_fnc_index;
		if(_index != -1) then
		{
			_illegal = _illegal + ((_x select 1) * ((__GETC__(gsn_sell_array) select _index) select 1));
		};
	} foreach _invs;
	if(_illegal > 6000) then
	{
		[[getPlayerUID _civ,_civ getVariable["realname",name _civ],"195"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
		[[_civ],"life_fnc_wantedFetchForCivilian",_civ,false] spawn life_fnc_MP;
	};

	[[getPlayerUID _civ,_civ getVariable["realname",name _civ],"194"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
	[[_civ],"life_fnc_wantedFetchForCivilian",_civ,false] spawn life_fnc_MP;
