...
if(_curTarget isKindOf "House_F" && playerSide == independent) then
{
	_jaws = missionNamespace getVariable (["jaws",0] call life_fnc_varHandle);
	if(_jaws > 0) then
	{
		_Btn1 ctrlSetText localize "STR_vInAct_JawsOfLife";
		_Btn1 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_jawsOfLife; closeDialog 0;";
		_Btn1 ctrlShow true;
	}
	else
	{
