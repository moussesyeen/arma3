...
if(_action) then {
	...
	[(round((_houseCfg select 0)/2)),1,0] call life_fnc_handleMoney;
	_index = gsn_life_vehicles find _house;
	if(_index != -1) then {
		gsn_life_vehicles set[_index,-1];
		gsn_life_vehicles = gsn_life_vehicles - [-1];
	};

	_index = [str(getPosATL _house),gsn_life_houses] call TON_fnc_index;
	if(_index != -1) then {
		gsn_life_houses set[_index,-1];
		gsn_life_houses = gsn_life_houses - [-1];
	};
