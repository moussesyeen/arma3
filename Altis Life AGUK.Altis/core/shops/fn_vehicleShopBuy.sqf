...
//Maxi 2/9/2015 give all vehicles 1 toolkit in a backpack automatically!
if(!(_className in ["B_UAV_01_F","B_Quadbike_01_F"])) then {
	_vehicle addBackpackCargoGlobal ["B_FieldPack_cbr_Repair",1]; 
};

//SUV's will no longer flip
if(_className == "C_SUV_01_F") then {
	_vehicle setCenterOfMass [-0.010813,-0.506166,-0.7];
};

[_vehicle] call life_fnc_ropeBlock;  //Blocks the rope for vehicles

//Side Specific actions.
switch(playerSide) do {
	case west: {
		[_vehicle,"cop_offroad",true] spawn life_fnc_vehicleAnimate;
		_vehicle addItemCargoGlobal ["FirstAidKit",2]; //Maxi 2/9/2015 first aid kits for medic vehicles!
	};
