	...
	if(typeName _type == typeName []) then {
		for "_i" from 0 to (count _type)-1 do
		{
			switch((_type select _i)) do
			{
				...
				case 3: {if(life_capture_point != -1) then {hintSilent parseText format["%1", _message];};};
				case 4: {diag_log format["%1", _message]}; //I added this
			};
		};
	} else {
		switch (_type) do
		{
			...
			case 2: {titleText[_msg,"PLAIN"];};
			case 3: {if(life_capture_point != -1) then {hintSilent parseText format["%1", _message];};};
			case 4: {diag_log format["%1", _message]}; //And this.
		};
	};
};
