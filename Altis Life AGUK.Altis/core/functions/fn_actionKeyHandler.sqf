...
if(_curTarget isKindOf "House_F" && {player distance _curTarget < 12} OR ((nearestObject [[7912.02,16148.6,0.396378],"Land_i_Barracks_V2_F"]) == _curTarget OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _curTarget)) exitWith {
...
{
	if((_curTarget GVAR["restrained",false]) && {!(player GVAR["restrained",false])} && !dialog && playerSide == west) then 
	{
		[_curTarget] call life_fnc_copInteractionMenu;
	};
	if((_curTarget GVAR["restrained",false]) && {!(player GVAR["restrained",false])} && !dialog && playerSide == civilian) exitWith
	{
