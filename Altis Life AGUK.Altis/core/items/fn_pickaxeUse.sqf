...
switch (true) do {
	...
	case (player distance (getMarkerPos "oil_rig_1") < 40) : {_mine = ["oil_unprocessed",3];};
	case (player distance (getMarkerPos "oil_rig_2") < 50): {_mine = ["oil_unprocessed",3];};
	default {""};
};
//Mine check
...
for "_i" from 0 to 2 do {
	...
	player say3D "PickaxeUseSound"; //asound
	sleep 2.5;
};
